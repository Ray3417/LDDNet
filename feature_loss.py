...
class FeatureLoss(torch.nn.Module):
    ...
    def forward(
        ...
        # else:
        #     y_hat_mask_unfolded = self._unfold(mask_src, kernels_radius)
        #     y_hat_unfolded = torch.abs(y_hat_mask_unfolded[:, :, kernels_radius, kernels_radius, :, :].view(N, C, 1, 1, height_pred, width_pred) - y_hat_mask_unfolded)
